...
#### Infisical secrets
All credentials and API keys are stored in Infisical under `/media` and injected at runtime.

- Ensure your project ID is available as `INFISICAL_PROJECT_ID`.
- Verify secrets exist at `/media` (for example `USERNAME`, `PASSWORD`, `SONARR_API_KEY`).

### 4. Data Directory Structure
```bash
sudo mkdir -p /data/{downloads/{qbittorrent/{completed,incomplete,torrents}},movies,shows,music,books,youtube}
...
4. Port: `8096`
5. Click "Next"
6. Sign in with your Infisical-managed credentials (`USERNAME`/`PASSWORD` in `/media`)
...
|---------|-----|---------------------|
| Sonarr | http://YOUR_SERVER:8989 | Infisical-managed credentials |
| Radarr | http://YOUR_SERVER:7878 | Infisical-managed credentials |
| Lidarr | http://YOUR_SERVER:8686 | Infisical-managed credentials |
| Prowlarr | http://YOUR_SERVER:9696 | Infisical-managed credentials |
| Bazarr | http://YOUR_SERVER:6767 | Infisical-managed credentials |
| qBittorrent | http://YOUR_SERVER:8080 | Infisical-managed credentials |
| Jellyfin | http://YOUR_SERVER:8096 | Infisical-managed credentials |
...
4. Configure with your Infisical-managed credentials
