...
description: "For any Sonarr Season 0 special that is really a film, add/verify a Radarr entry and search there first rather than grabbing the movie file through Sonarr — check Radarr/TMDB for EVERY special, not just long ones"
metadata: 
  node_type: memory
  type: feedback
...
When a Sonarr Season 0 special turns out to really be a film, and no Radarr entry exists for it yet, add it to Radarr first and solve the gap there — not by searching/grabbing it as a Sonarr episode.

**Runtime threshold removed 2026-09-18.** This rule previously fired only for specials "≥60min" (later "≥~50min"). The owner's call: *"length isn't a good enough judgement to determine what an episode is."* Runtime is no longer a classifier or a filter anywhere in this workflow — **check Radarr/TMDB for EVERY Season 0 item regardless of length.** The evidence is in this repo twice: `.hack`'s "Online Jack" (nine 2-4min specials) is genuine narrative content, and Gundam 0083's 4min/12min bonus shorts were skipped across an entire audit pass because the query filtered on `runtime>=60`. `.hack` also had *short* specials already sitting in Radarr under different titles — so length told you nothing about whether it was worth looking. See `media/docs/SONARR_ACQUISITION_PROCESS.md` steps 2c and 3.
...
**Root cause of the repeated mistake:** TVDB does not model movies as separate entries the way TMDB does — a "no TVDB match" on movie-length content is not evidence it lacks a separate identity, it's evidence the wrong database was checked. **The rule is not "check Radarr only when Sonarr's TVDB lookup fails" — check Radarr/TMDB first for every Season 0 item and skip the TVDB lookup entirely**, since TVDB will almost always give a false "stays as a TV special" answer for real films. (This sentence previously said "for anything ≥~50min runtime"; the runtime qualifier was removed 2026-09-18 — see the note at the top.)
