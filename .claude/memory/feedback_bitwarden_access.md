...
**How to apply:** For infisical authentication, the correct approach is the user manually runs `infisical login --domain http://192.168.20.22:8081` in their terminal (browser-based OAuth). Claude then uses the resulting session. If the session expires, prompt the user to re-run the login command — do not suggest Bitwarden automation as a shortcut.
