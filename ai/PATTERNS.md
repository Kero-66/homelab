...
| `curl -X PUT/POST/DELETE http://sonarr.home/api/v3/...` or `http://radarr.home/api/v3/...` (no base path) | **Both apps are served under a base path** — `/sonarr` and `/radarr` — and return **307** to e.g. `/sonarr/api/v3/...`. `curl` does not follow a redirect without `-L`, and even with `-L` a redirected PUT/POST can lose its method or body, so **the write silently never happens**. It returns `307`, not an error, so it reads like success. Every `curl -sL` GET in this repo works only because `-L` follows the redirect, which is why this stayed hidden. Confirmed 2026-09-18 twice: two Radarr `DELETE`s "succeeded" with 307 while both movies stayed in the library, and a Sonarr unmonitor `PUT` returned 307 while the episode stayed `monitored:true` | Use the full base path — `http://sonarr.home/sonarr/api/v3/...` / `http://radarr.home/radarr/api/v3/...` — with `-H "X-Api-Key: $SONARR_KEY"`. **Always re-query after a write to confirm it applied**; a 3xx on a write means it did not. A successful Sonarr write returns `202`, Radarr `200`/`202` |
