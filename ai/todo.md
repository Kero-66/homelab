...
94 | prowlarr_indexer_category_scope_review | Review Prowlarr indexers' **sync categories** config — several searches this session (Robotech S2/S3, Zoids S1, Gasaraki/Big O/Macross II single episodes, various Tekkaman Blade/.hack movies) came back with zero results across all indexers, more often than expected. Worth checking whether the sync categories each indexer is scoped to in Prowlarr are too restrictive (e.g. excluding anime/OVA/older-TV categories that would actually have these releases) rather than assuming the content simply isn't indexed anywhere. | Kieran | 2026-08-20 | open | truenas/stacks/prowlarr/
