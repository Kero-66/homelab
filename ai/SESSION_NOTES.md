...
**Fix**: reran `truenas/scripts/../media/scripts/watch_orders/build_playlist.py tekkaman.json` (`JELLYFIN_KEY=$(...) python3 build_playlist.py tekkaman.json`) — it references content by title/season, not raw item IDs, and its delete-and-recreate approach uses the (working) create-playlist endpoint instead of the broken add-items one. Recreated cleanly at the same playlist ID, 60/60 items, correct order (Prelude first, Twin Blood/Burning Clock/Missing Link right after "The Burning Life", before Virgin Memory), subtitle confirmed present on the first item via the playlist reference.

**Also confirms** the playlist's `OwnerUserId` is a *different* Jellyfin user than kero66 (`b9e55ddf2d3299166db93b5cce7a62ef` vs kero66's `1ecfce63139f4501a4d498e372e1ee3d`) — same "first user from `/Users`" default-owner bug already documented for `smartlist.py`, now confirmed to also apply to `build_playlist.py`'s `user_id()` function (`media/scripts/watch_orders/build_playlist.py:110-112`). Not fixed this session — worth doing if it causes a real problem (e.g. a per-user watched/favorite state on the playlist item itself), but functionally harmless since anyone can still view an owned-by-another-user playlist.
