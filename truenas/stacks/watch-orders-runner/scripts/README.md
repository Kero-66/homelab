...
| Star Wars: The Clone Wars | Manual (`clone_wars.json`) | **Chronological (in-universe) order** — the franchise's canonical air-order-≠-story-order case. Source of record: the official StarWars.com chronological episode order article, fetched and transcribed in full 2026-09-18 (all 134 entries). Wikipedia confirms the official order exists but does not reproduce it. The 134 entries are collapsed into 27 contiguous ranges; the collapse was **verified programmatically to expand back to the official 134-entry sequence exactly**, so the compression cannot have introduced a reordering. Full order encoded including unowned seasons, so S3-S7 splice in with no edit | 39 resolve today (S1E6-E22 + S2 = exactly what is owned), 17 entries reserved |
