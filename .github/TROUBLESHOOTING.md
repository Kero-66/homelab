...
# Stop homepage (using env files for parst-time substitution)
```bash
cd apps/homepage
docker compose \
  --profile homepage  \
...
# Stop media stack (using env files for parst-time substitution)
```bash
cd media
docker compose \
  --profile media  \
...
# Alternatively, use the qBittorrent API (login + unban) if available
...
- This memory is **not** committed to the repository and does not contain secrets; it is for the assistant's own use to be more efficient.
...
---
If you'd like changes (formatting, extra commands, or a different location), tell me and I'll update the file and open a PR.